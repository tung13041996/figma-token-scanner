...
async function scan(pageIds = null){
                    ...
                    for(const seg of segs){
                        ...
                        if(seg.fontName){
                            ...
                            upsert(FONTS, `${family}|||${weight}`,
                                ...
                                e => {
                                    e.count++;
                                    if(!e.style && tsInfo) e.style = tsInfo;
                ...
                for(let fi = 0; fi < fills.length; fi++){
                    ...
                    upsert(COLORS, key,
                        ...
                        e => {
                            e.count++;
                            if(!e.variable && vi) e.variable = vi;
                            if(!e.style && paintStyle) e.style = paintStyle;
                            addOccurrence(e, page.name, node.name);
                        }
                    );
                }
    ...
    figma.ui.postMessage({
        ...
        data: {
            ...
            spacings: [...SPACES.entries()].map(([k, e]) => {
                const {_pages, types, ...rest} = e;
                return {...rest, key: k, types: [...types], pageCount: _pages.size};
    ...
    for(const node of nodes){
        ...
        if(tokenType === 'font' && node.type === 'TEXT'){
            if(node.fontName !== figma.mixed){
                ...
                // Mixed text node: check each segment individually
                try{
                    hit = node.getStyledTextSegments(['fontName']).some(s =>
                        s.fontName && s.fontName.family === family && s.fontName.style === weight
                    );
                }catch{
                }
            }
            ...
            if(node.fontSize !== figma.mixed){
                ...
                try{
                    hit = node.getStyledTextSegments(['fontSize']).some(s => s.fontSize === value);
                }catch{
                }
            }
